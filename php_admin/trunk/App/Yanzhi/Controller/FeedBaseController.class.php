...
class FeedBaseController extends PublicController
{
  ...
  public function view()
  {
    ...
    else
    {
      ...
      $sco->pager->parameter = array('id' => $fid,'tab' => 'tab-feed-scores');
  ...
  public function del()
  {
    ...
    elseif(!$mod->where(array('id' => $id))->limit(1)->delete())
    {
      $this->error('操作失败');
    }
    else
  ...
  public function comments()
  {
    ...
    $dat = array();
    $map = $mod->get_filters();//筛选搜索
    $dat['list'] = $mod->plist(C('ITEMS_PER_PAGE') ?: 60,$map)->lists('','createtime desc,id desc');//C('ITEMS_PER_PAGE')
    $this->pager = $mod->pager;
    $this->page = $dat['page_html'] = $mod->pager->show();
    $dat['comment_types'] = $mod->comment_types ?: array();
