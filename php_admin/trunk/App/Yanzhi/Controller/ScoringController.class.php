...
class ScoringController extends PublicController
{
  ...
  public function score_logs()
  {
    ...
    $dat = array();
    $map = $mod->get_filters();
    $dat['list'] = $mod->plist(100,$map)->lists('','assign_time desc,id desc') ?: array();
    $this->pager = $mod->pager;
    $this->page = $dat['page_html'] = $mod->pager->show();
    if($ids = array_unique(array_column($dat['list'],'feed_id')))
    {
      $dat['feeds'] = D('FeedBase')->where(array('id' => array('in',$ids)))->klist() ?: array();
    }
    if($ids = array_unique(array_column($dat['list'],'uid')))
    {
      $dat['ranges'] = D('UserScoring')->alias('us')
        ->field(array('us.*','r.name' => 'range_name','r.work_start','r.work_end'))
        ->join('left join __USER_SCORING_RANGE__ r on r.id = us.range_id')
        ->where(array('us.uid' => array('in',$ids)))->klist('uid') ?: array();
    }
    $this->data = $dat;
    //die(json_encode($dat));
    $this->display();
  ...
  // 打分系统消息
  protected function scoring_msg($uid,$feed = array())
  {
