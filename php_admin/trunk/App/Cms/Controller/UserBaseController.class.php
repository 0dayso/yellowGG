...
class UserBaseController extends PublicController
{
  ...
  public function view()
  {
    ...
    else
    {
      ...
      $dat['accusation_admins']  = $acc->get_accusation_admins($dat['accusation_logs'],'nickname');
      // 被举报记录
      $rep = D('ReportBase');
  ...
  public function get_chat_log()
  {
    ...
    $dat = array();
    $mod = D('ChatLogBase');
    $dat['list'] = $mod->get_chat_log_union($map,$day)->select() ?: array();
    is_array($dat['list']) && $dat['list'] = array_reverse($dat['list']);
    $dat['list'] = $mod->format_text_all($dat['list']);
    $this->data = $dat;
    $this->ret['data'] = $dat;
  ...
  public function accusation_logs()
  {
    ...
    $dat['accusation_admins']  = $mod->get_accusation_admins($dat['list'],'nickname');
    $this->data = $dat;
    //die(json_encode($dat));
    $this->display();
  }
  ...
  {
    $mod = D(CONTROLLER_NAME);
    $api = 'http://api.chujian.im/v1.6/index.php/';
    ...
    else
    {
      $dat['feed_id'] = $fid;
      //die(json_encode($dat));
      $jss = $this->http($api.'auth/reg',$dat,'POST');
      ...
      if($uid = (int)$arr['uid'])
      {
        ...
        //$this->http($api.'lbs/update_location',$loc,'POST');//不上报位置
