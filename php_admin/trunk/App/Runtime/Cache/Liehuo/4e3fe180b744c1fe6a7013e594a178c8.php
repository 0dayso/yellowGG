...
<div id="content">
  <div class="container-fluid">
    <div class="row-fluid">
      <div class="clearfix">
        <form action="<?php echo U('offline_data');?>" method="POST" enctype="multipart/form-data" class="form-inline pull-left">
          <div class="form-group">
            <label>选择文件:</label>
          ...
          <div class="btn-group">
            <span class="btn btn-white">导入数：<?php echo count($data['phones']);?></span>
            <span class="btn btn-white">匹配数：<?php echo count($data['list']);?></span>
          </div>
        </form>
        <form id="form_save" action="<?php echo U('offline_data_save');?>" method="POST" enctype="multipart/form-data" class="form-inline pull-right">
          <div class="form-group">
            <label>广告序列号:</label>
            <input type="text" name="serial" value="<?php echo ($data['serial']); ?>" placeholder="如：dt1" class="form-control">
          </div>
          <div class="form-group">
            <select name="pkg_id" class="form-control">
              <option value="">安装包</option>
<?php if(is_array($data['pkgs'])): $i = 0; $__LIST__ = $data['pkgs'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; $oss = ['安卓','IOS']; ?>
              <option value="<?php echo ($v['id']); ?>"<?php echo $_REQUEST['pkg_id'] == (string)$v['id'] ? ' selected' : ''; ?>><?php echo ($oss[$v['pkg_device']]); ?> <?php echo ($v['pkg_name']); ?> (<?php echo ($v['pkg_version']); ?>)</option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
          </div>
          <button type="submit" class="btn btn-success">确认提交</button>
          <a href="<?php echo U('analy/user_stat?rdrs_type=dt');?>" class="btn btn-primary">行为统计</a>
        </form>
      </div>
      <hr>
...
<?php if(is_array($data['list'])): $i = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; $sex = $v['sex']; ?>
            <tr class="gradeX form-sub">
              <td>
                <label>
<?php if(!$data['isok']): ?><input type="checkbox" name="ids[]" value="<?php echo ($v['uid']); ?>" <?php echo ($data['isok'] ? 'disabled' : 'checked'); ?> form="form_save"><?php endif; ?>
                  <?php echo ($v['uid'] ?: ''); ?>
                </label>
              </td>
              <td><a href="<?php echo U('UserBase/view?uid='.$v['uid']);?>" target="_blank"><?php echo ($v['nickname'] ?: ''); ?></a></td>
              <td><?php echo (C("USER_SEX_IS.$sex")); ?></td>
              <td class="field-phone" data-phone="<?php echo ($v['phone']); ?>"><?php echo ($v['phone']); ?></td>
              <td><?php echo ($v['score']); ?></td>
              <td<?php echo $v['device_id'] && count($data['devices'][$v['device_id']]) >= 2 ? ' class="text-danger"' : ''; ?>><?php echo ($v['device_id'] ?: '-'); ?></td>
              <td><?php echo date('Y-m-d H:i:s',$v['reg_time']);?></td>
              <td><?php echo ($v['active_time'] ? date('Y-m-d H:i:s',$v['active_time']) : '-'); ?></td>
              <td><?php echo ($v['pkg_channel']); ?></td>
...
<script>
window.offline_phones = <?php echo json_encode($data['phones'] ?: []);?>;
