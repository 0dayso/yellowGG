    ...
    <div class="row-fluid">
      <div class="clearfix">
        ...
        <div class="pull-right">
          <a href="<?php echo U('discount_edit');?>" class="btn btn-success">新增活动</a>
...
<?php if(is_array($data['list'])): $i = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr class="gradeX">
              <td><?php echo ($v['id']); ?></td>
              <td><?php echo ($data['goods_list'][$v['goods_id']]['name'] ?: $v['goods_id']); ?></td>
              <td>
                <div class="td-content"><?php echo ($v['remark']); ?></div>
              </td>
              <td>
                <div class="td-content">
<?php $als = is_array($v['attrs']) ? $v['attrs'] : (json_decode($v['attrs'],true) ?: []); echo implode('',array_map(function($k,$v) { if(!$v) return ''; return $k.':'.$v.'<br>'; },array_keys($als),$als)); ?>
                </div>
              </td>
              <td><?php echo ($v['discount_stime'] ? date('Y-m-d H:i:s',$v['discount_stime']) : '-'); ?></td>
              <td><?php echo ($v['discount_etime'] ? date('Y-m-d H:i:s',$v['discount_etime']) : '-'); ?></td>
              <td><i class="fa <?php echo boolval($v['discount_stime'] <= time() && $v['discount_etime'] >= time()) ? 'fa-check text-success' : 'fa-close text-danger';?>"></i></td>
              <td class="text-nowrap">
                <a href="<?php echo U('discount_edit?id='.$v['id']);?>" class="btn btn-primary">编辑</a>
                <a href="<?php echo U('discount_del?id='.$v['id']);?>" class="btn btn-danger" onsubmit="return confirm('真的要这么做？')">删除</a>
              </td>
