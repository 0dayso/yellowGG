...
<div id="content">
  <div class="container-fluid">
    <div class="row-fluid">
      <div class="clearfix">
        <form action="<?php echo U();?>" method="POST" enctype="multipart/form-data" class="form-inline pull-left">
          <div class="form-group">
            <label>选择照片</label>
        ...
        <form id="form_save" action="<?php echo U('snap_send');?>" method="POST" enctype="multipart/form-data" class="form-inline pull-right">
...
<?php if(is_array($data['list'])): $i = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; $sex = $v['sex']; ?>
            <tr class="gradeX form-sub">
              <td>
                <label>
                  <input type="checkbox" name="ids[<?php echo ($v['uid']); ?>]" value="<?php echo ($v['uid']); ?>"<?php echo ($v['is_sent'] ? '' : ' checked'); ?> form="form_save">
                  <?php echo ($v['uid'] ?: ''); ?>
                </label>
              </td>
              ...
              <td>
                <img src="__IM_IMAGE_URL_ROOT__<?php echo ($v['resource']); ?>" alt="<?php echo ($v['resource']); ?>" class="zoom" style="max-height:80px;">
                <input type="hidden" name="resource[<?php echo ($v['uid']); ?>]" value="<?php echo ($v['resource']); ?>" form="form_save">
              </td>
