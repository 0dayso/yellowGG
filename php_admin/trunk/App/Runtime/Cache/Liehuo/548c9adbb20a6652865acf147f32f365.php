          ...
          <li>
            ...
            <ul class="nav nav-second-level collapse">
              ...
              <li>
                ...
                <ul class="nav nav-third-level collapse">
                  <li><a class="auto-openli" href="<?php echo U('analy/get_n');?>">基础数据</a></li>
...
<div id="content">
  <div class="container-fluid">
    <div class="row-fluid">
      <div class="clearfix">
        <form action="<?php echo U();?>" method="GET" class="form-inline pull-left<?php echo ($hide_filter ? ' hide' : ''); ?>">
          <input type="hidden" name="act" value="filter">
          <input type="hidden" name="uid" value="<?php echo ($_REQUEST['uid']); ?>">
          <input type="hidden" name="type" value="<?php echo ($_REQUEST['type']); ?>">
          <input type="hidden" name="rdrs_type" value="<?php echo ($_REQUEST['rdrs_type']); ?>">
          <input type="hidden" name="user_type" value="<?php echo ($_REQUEST['user_type']); ?>">
          <input type="hidden" name="ad_serial" value="<?php echo ($_REQUEST['ad_serial']); ?>">
          <input type="hidden" name="time_type" value="<?php echo ($time_type ?: $_REQUEST['time_type']); ?>">
...
<?php if(in_array(ACTION_NAME,['stat_retention','stat_retention_detail','hourly_slide'])): elseif(in_array(ACTION_NAME,['been_slide','been_thumb','match_rate','daily_chat'])): ?>
          <div class="form-group">
            <select name="user_type" class="form-control">
              <option value="">用户类型</option>
              <option value="0"<?php echo $_REQUEST['user_type'] == '0' ? ' selected' : ''; ?>>老用户</option>
              <option value="1"<?php echo $_REQUEST['user_type'] == '1' ? ' selected' : ''; ?>>新用户</option>
            </select>
          </div>
<?php else: ?>
<?php if($data['devices']): ?><div class="form-group">
            <select name="device" class="form-control">
              <option value="">全部设备</option>
<?php if(is_array($data['devices'])): $i = 0; $__LIST__ = $data['devices'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"<?php echo $_REQUEST['device'] == (string)$key ? ' selected' : ''; ?>><?php echo ($v); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
          </div><?php endif; ?>
<?php if($data['channels']): ?><div class="form-group">
            <select name="ch_id" class="form-control">
              <option value="">全部渠道</option>
<?php if(is_array($data['channels'])): $i = 0; $__LIST__ = $data['channels'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"<?php echo $_REQUEST['ch_id'] == (string)$key ? ' selected' : ''; ?>><?php echo ($v['ch_name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
          </div><?php endif; ?>
<?php if($data['pkg_channels']): ?><div class="form-group">
            <select name="pkg_channel" class="form-control">
              <option value="">全部包渠道</option>
<?php if(is_array($data['pkg_channels'])): $i = 0; $__LIST__ = $data['pkg_channels'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"<?php echo $_REQUEST['pkg_channel'] == (string)$key ? ' selected' : ''; ?>><?php echo ($v['ch_serial']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
          </div><?php endif; ?>
<?php if($data['advers']): ?><div class="form-group">
            <select name="adver_id" class="form-control">
              <option value="">全部广告</option>
<?php $oss = ['安卓','IOS']; ?>
<?php if(is_array($data['advers'])): $i = 0; $__LIST__ = $data['advers'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; $pkg = $data['packages'][$v['pkg_id']] ?: []; ?>
              <option value="<?php echo ($key); ?>"<?php echo $_REQUEST['adver_id'] == (string)$key ? ' selected' : ''; ?>><?php echo ($v['name'] ?: $v['ch_serial']); ?> (<?php echo ($oss[$pkg['pkg_device']]); ?> <?php echo ($pkg['pkg_version']); ?>)</option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
          </div><?php endif; ?>
<?php if($data['packages']): ?><div class="form-group hide">
            <select name="pkg_id" class="form-control">
              <option value="">全部包版本</option>
<?php $oss = ['安卓','IOS']; ?>
<?php if(is_array($data['packages'])): $i = 0; $__LIST__ = $data['packages'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"<?php echo $_REQUEST['pkg_id'] == (string)$key ? ' selected' : ''; ?>><?php echo ($oss[$v['pkg_device']]); ?> <?php echo ($v['pkg_name']); ?> (<?php echo ($v['pkg_version']); ?>)</option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
          </div><?php endif; endif; ?>
<?php if($data['sexs']): ?><div class="form-group">
            <select name="sex" class="form-control">
              <option value="">性别</option>
<?php if(is_array($data['sexs'])): $i = 0; $__LIST__ = $data['sexs'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"<?php echo $_REQUEST['sex'] == (string)$key ? ' selected' : ''; ?>><?php echo ($v); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
          </div><?php endif; ?>
<?php if($data['freq_types']): ?><div class="form-group">
            <select name="pay_freq_type" class="form-control">
              <option value="">付费频率</option>
<?php if(is_array($data['freq_types'])): $i = 0; $__LIST__ = $data['freq_types'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"<?php echo $_REQUEST['pay_freq_type'] == (string)$key ? ' selected' : ''; ?>><?php echo ($v); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
          </div><?php endif; ?>
<?php if(ACTION_NAME == 'daily_user'): ?><div class="form-group">
            <input type="text" name="kwd" value="<?php echo ($_REQUEST['kwd']); ?>" placeholder="用户ID..." class="form-control">
          </div><?php endif; ?>
          <button type="submit" class="btn btn-primary">搜索</button>
          <a href="<?php echo U('',$all_params ?: []);?>" class="btn btn-white">全部</a>
        </form>
        <div class="pull-right">
<?php if(ACTION_NAME == 'adver_stat'): if($_REQUEST['rdrs_type'] == 'dt'): ?><a href="<?php echo U();?>" class="btn btn-success">所有数据</a>
          <a href="<?php echo U('update_offline_daily');?>" class="btn btn-primary tip" data-original-title="请将前一天的地推用户导入完毕再更新，更新很慢请耐心等待，请勿重复刷新，避免频繁操作">更新数据</a>
<?php else: ?>
          <a href="<?php echo U('?rdrs_type=dt');?>" class="btn btn-success">地推数据</a><?php endif; endif; ?>
          <span class="btn btn-white">记录数：<?php echo ($pager ? $pager->totalRows : count($data['list'])); ?></span>
        </div>
      </div>
      <div class="table-responsive">
        <table class="table table-bordered table-striped table-hover text-center an-auto-rowspan <?php echo ($table_class ?: ''); ?>">
          <thead>
            <tr>
<?php if(is_array($data['cols'])): $i = 0; $__LIST__ = $data['cols'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$k): $mod = ($i % 2 );++$i;?><th><?php echo ($k); ?></th><?php endforeach; endif; else: echo "" ;endif; ?>
            </tr>
          </thead>
          <tbody>
<?php if(is_array($data['list'])): $i = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
<?php if(is_array($data['cols'])): $i = 0; $__LIST__ = $data['cols'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$k): $mod = ($i % 2 );++$i;?><td<?php echo in_array($k,$auto_rowspan ?: []) ? ' class="auto-rowspan"' : ''; ?>><?php echo ($v[$k]); ?></td><?php endforeach; endif; else: echo "" ;endif; ?>
