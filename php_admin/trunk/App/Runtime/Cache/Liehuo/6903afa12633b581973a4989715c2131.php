...
<div id="content">
  <div class="container-fluid">
    <div class="clearfix">
      <form action="<?php echo U();?>" method="GET" class="form-inline">
        ...
        <div class="input-prepend input-group">
          ...
          <input type="text" value="" class="form-control date-range"
                 data-stime="<?php echo ($_REQUEST['stime'] ?: date('Y-m-d')); ?>"
                 data-etime="<?php echo ($_REQUEST['etime']); ?>"
                 data-limit="31">
          <input type="hidden" name="stime" value="<?php echo ($_REQUEST['stime'] ?: date('Y-m-d')); ?>">
          <input type="hidden" name="etime" value="<?php echo ($_REQUEST['etime']); ?>">
        </div>
        <div class="form-group" style="display:none;">
          <select name="day" class="form-control" disabled>
            <option value="1"<?php echo $_REQUEST['day'] == '1' ? ' selected' : ''; ?>>最近1天</option>
            <option value="3"<?php echo $_REQUEST['day'] == '3' ? ' selected' : ''; ?>>最近3天</option>
            <option value="7"<?php echo $_REQUEST['day'] == '7' ? ' selected' : ''; ?>>最近7天</option>
            <option value="15"<?php echo $_REQUEST['day'] == '15' ? ' selected' : ''; ?>>最近15天</option>
            <option value="30"<?php echo $_REQUEST['day'] == '30' ? ' selected' : ''; ?>>最近30天</option>
          </select>
        </div>
        <div class="form-group">
          <select name="texttype" class="form-control">
            <option value="">消息类型</option>
            <option value="1"<?php echo $_REQUEST['texttype'] == '1' ? ' selected' : ''; ?>>文字</option>
            <option value="2"<?php echo $_REQUEST['texttype'] == '2' ? ' selected' : ''; ?>>图片</option>
            <option value="3"<?php echo $_REQUEST['texttype'] == '3' ? ' selected' : ''; ?>>表情</option>
            <option value="4"<?php echo $_REQUEST['texttype'] == '4' ? ' selected' : ''; ?>>语音</option>
            <option value="5"<?php echo $_REQUEST['texttype'] == '5' ? ' selected' : ''; ?>>位置</option>
          </select>
        </div>
        <div class="form-group">
      ...
      <form id="form_main" action="<?php echo U('closure_bat');?>" method="POST" class="form-inline pull-right" onsubmit="return confirm('真的要这么做？')">
    ...
    <hr>
    <div class="chat-logs clearfix<?php echo ($show_inline ? ' chat-logs-inline' : ''); ?>">
<?php if(is_array($data['list'])): $i = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; $is_self = $v['sender'] == $_REQUEST['uid']; $ser = $data['users'][$v['sender']] ?: []; $rer = $data['users'][$v['recver']] ?: []; $sex_color = ['success','danger','waring']; ?>
  <div class="media chat-msg<?php echo ($is_self ? ' chat-myself' : ''); ?>" data-id="<?php echo ($v['smsid']); ?>">
    <div class="media-body">
      <p class="media-heading chat-info">
        <label>
          <input type="checkbox" name="ids[<?php echo ($v['sender']); ?>]" value="<?php echo ($v['sender']); ?>" form="form_main">
          <small class="chat-time"><?php echo ($v['time']); ?></small>
        </label>
        <a href="<?php echo U('view?uid='.$v['sender'].'&tab=tab-user-chat');?>" target="_blank" class="label label-<?php echo ($sex_color[$ser['sex']] ?: 'primary'); ?> popover-avatar" data-placement="top"><?php echo ($v['sender']); ?></a>
        <b class="label label-danger"><?php echo boolval($ser['vip_level'] && $ser['vip_valid_end'] >= time()) ? 'v' : ''; echo ($ser['glory_grade'] ?: ''); ?></b>
        对
        <a href="<?php echo U('view?uid='.$v['recver'].'&tab=tab-user-chat');?>" target="_blank" class="label label-<?php echo ($sex_color[$rer['sex']] ?: 'primary'); ?> popover-avatar" data-placement="top"><?php echo ($v['recver']); ?></a>
        <b class="label label-danger"><?php echo boolval($rer['vip_level'] && $rer['vip_valid_end'] >= time()) ? 'v' : ''; echo ($rer['glory_grade'] ?: ''); ?></b>
        说：
      </p>
      <div class="chat-text">

<?php if($v['texttype'] == '2' && $v['text_json']['thumbnailPhotoUrl']): ?><img src="<?php echo ($v['text_json']['thumbnailPhotoUrl']); ?>" data-src="<?php echo ($v['text_json']['originPhotoUrl'] ?: $v['text_json']['thumbnailPhotoUrl']); ?>" class="zoom">

<?php elseif($v['texttype'] == '3' && $v['text_json']['emoticonUrl']): ?>
        <img src="<?php echo ($v['text_json']['emoticonUrl']); ?>" title="<?php echo ($v['text_json']['emoticonName']); ?>">

<?php elseif($v['texttype'] == '4' && $v['text_json']['voiceUrl']): ?>
        <a href="<?php echo ($v['text_json']['voiceUrl']); ?>" target="_blank" class="label label-success jplayer-voice">[语音] <?php echo ($v['text_json']['voiceTime']); ?>s</a>

<?php elseif($v['texttype'] == '5' && $v['text_json']['locationAddress']): ?>
        <img src="<?php echo ($v['text_json']['locationImageUrl']); ?>" class="zoom">
        <br>[位置] <?php echo ($v['text_json']['locationAddress']); ?>

<?php elseif($v['texttype'] == '100' && $v['text_json']['type'] == '7'): ?>
<?php $jso = $v['text_json']['json']; ?>
        <div class="msg-articles clearfix articles-1">
          <ul class="list-group">
            <li class="list-group-item">
              <div class="msg-article-tit">
<?php if($jso['sessionThumbImageUrl']): ?><img src="<?php echo ($jso['sessionThumbImageUrl']); ?>" class="zoom"><?php endif; ?>
              </div>
<?php if($jso['comment']): ?><p class="msg-article-desc"><?php echo ($jso['comment']); ?></p><?php endif; ?>
            </li>
          </ul>
        </div>

<?php elseif($v['texttype'] == '100' && $v['text_json']['type'] == '8'): ?>
<?php $jso = $v['text_json']['json']; $cnt = count($jso); $art = $jso[0] ?: []; unset($jso[0]); ?>
        <div class="msg-articles clearfix articles-<?php echo ($cnt); ?>">
          <ul class="list-group">
            <a<?php echo $art['link'] ? (' href="'.$art['link'].'"') : ''; ?> target="_blank" class="list-group-item">
              <div class="msg-article-tit">
                <h4><?php echo ($art['title'] ?: '运营消息'); ?></h4>
<?php if($art['thumb']): ?><img src="<?php echo ($art['thumb']); ?>" class="zoom"><?php endif; ?>
              </div>
<?php if($art['desc']): ?><p class="msg-article-desc"><?php echo ($art['desc']); ?></p><?php endif; ?>
              <p class="msg-article-read-more">查看全文</p>
            </a>
<?php if(is_array($jso)): $i = 0; $__LIST__ = $jso;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$art): $mod = ($i % 2 );++$i;?><a class="list-group-item clearfix">
<?php if($art['thumb']): ?><img src="<?php echo ($art['thumb']); ?>" class="pull-right"><?php endif; ?>
              <?php echo ($art['title']); ?>
            </a><?php endforeach; endif; else: echo "" ;endif; ?>
          </ul>
        </div>

<?php elseif($v['texttype'] == '100' && $v['text_json']['res']): ?>
<?php $jso = $v['text_json'] ?: []; ?>
        <div class="msg-articles msg-snap clearfix articles-1">
          <ul class="list-group">
            <li class="list-group-item">
              <div class="msg-article-tit">
<?php if($jso['res']): ?><img src="<?php echo ($jso['res']); ?>" class="zoom"><?php endif; ?>
              </div>
<?php if($jso['txt'] || 1): ?><p class="msg-article-desc">【瞬间】<?php echo ($jso['txt']); ?></p><?php endif; ?>
            </li>
          </ul>
        </div>

<?php elseif($v['texttype'] == '100' && in_array($v['text_json']['type'],[10,11])): ?>
<?php $jso = $v['text_json']['json'] ?: json_decode($v['text_json']['text'],true) ?: []; $cnt = count($jso); if(!$jso) var_dump($v['text_json']); $hls = []; foreach($jso as $j) { $j['type'] == 'intent' && $j['name'] && $j['url'] = '#'.$j['name']; $url = trim($j['url']); $css = ($j['color'] ? ('color:#'.$j['color'].';') : '').($j['fontSize'] ? ('font-size:'.$j['fontSize'].'px;') : ''); if(in_array($j['type'],['text'])) { $hls[] = '<span'.($css ? (' style="'.$css.'"') : '').'>'.$j['content'].'</span>'; } elseif(in_array($j['type'],['link','intent'])) { $hls[] = '<a'.($url ? (' href="'.$url.'"') : '').($css ? (' style="'.$css.'"') : '').'>'.$j['content'].'</a>'; } } echo implode('',$hls); ?>

<?php elseif($v['text_json'] && $v['text_json']['text']): ?>
        <?php echo ($v['text_json']['text']); ?>

<?php else: ?>
        <?php echo ($v['text_html'] ?: $v['text']); endif; ?>
      </div>
    </div>
    ...
    <div class="pagination alternate">
      <ul>
        <li style="text-align: center;color:#00f"><?php echo ($pager->show()); ?></li>
      </ul>
    </div>
  </div>
</div>
