...
<?php if(is_array($sexs)): $i = 0; $__LIST__ = $sexs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"<?php echo $_REQUEST['sex'] == (string)$key ? ' selected' : ''; ?>><?php echo ($v); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        ...
        <div class="form-group">
          <input type="text" name="kwd" value="<?php echo ($_REQUEST['kwd']); ?>" placeholder="备注、用户ID..." class="form-control">
        </div>
        <button type="submit" class="btn btn-primary">搜索</button>
        <span class="btn btn-white pull-right">记录数：<?php echo ($pager->totalRows); ?></span>
        <a href="<?php echo U('?download=1',array_merge($_GET ?: [],['page_size' => 'export']));?>" class="btn btn-success pull-right">导出</a>
      </form>
      <div class="table-responsive">
        <table class="table table-bordered table-striped table-hover text-center">
          ...
          <tbody>
<?php if(is_array($data['list'])): $i = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; $mat = $data['matchs'][$v['uid']][$v['oid']] ?: []; ?>
            <tr>
              <td><?php echo ($v['id']); ?></td>
              <td>
                <a href="<?php echo U('view?uid='.$v['uid']);?>" target="_blank" class="label label-default"><?php echo ($v['uid'] ?: ''); ?></a>
              </td>
              <td>
                <div class="td-content popover-hover"><?php echo htmlspecialchars($data['users'][$v['uid']]['nickname']);?></div>
              </td>
              <td>
                <a href="<?php echo U('view?uid='.$v['oid']);?>" target="_blank" class="label label-default"><?php echo ($v['oid'] ?: ''); ?></a>
              </td>
              <td><?php echo ($mat['create_time'] ? date('Y-m-d H:i:s',$mat['create_time']) : '-'); ?></td>
              <td><?php echo ($v['add_time'] ? date('Y-m-d H:i:s',$v['add_time']) : '-'); ?></td>
