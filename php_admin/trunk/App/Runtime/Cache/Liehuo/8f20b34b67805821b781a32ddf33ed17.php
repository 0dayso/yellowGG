    ...
    <div class="row-fluid">
      <div class="clearfix">
        ...
        <div class="pull-right">
          <a href="<?php echo U('edit');?>" class="btn btn-success">新增文章</a>
...
<?php if(is_array($data['list'])): $i = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr class="gradeX">
              <td><?php echo ($v['id']); ?></td>
              <td><img src="<?php echo ($v[thumb]); ?>" style="max-width:120px;"></td>
              <td>
                <div class="td-content"><?php echo ($v['title']); ?></div>
              </td>
              <td>
                <div class="td-content"><?php echo ($v['desc']); ?></div>
              </td>
              <td><?php echo ($v['read_num']); ?></td>
              <td><?php echo ($v['create_time'] ? date('Y-m-d H:i:s',$v['create_time']) : '-'); ?></td>
              <td class="text-nowrap">
                <a href="<?php echo U('view?id='.$v['id']);?>" target="_blank" class="btn btn-success">查看</a>
                <a href="<?php echo U('edit?id='.$v['id']);?>" target="_blank" class="btn btn-primary">编辑</a>
                <a href="<?php echo U('del?id='.$v['id']);?>" class="btn btn-danger" onsubmit="return confirm('真的要这么做？')">删除</a>
                <a href="<?php echo U('Promo/mass_send?article_id='.$v['id']);?>" target="_blank" class="btn btn-success">群发</a>
              </td>
