...
<?php if(is_array($data['list'])): $i = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; $score_show = $v['score'] ?: ''; $score_css = 'default'; $score_show >= 1 && $score_css = 'primary'; $score_show >= 5 && $score_css = 'success'; $score_show >= 8 && $score_css = 'danger'; $score_timeout = $v['score_time'] ? ((int)$v['score_time'] - (int)$v['create_time']) : 0; $usr = $data['users'][$v['uid']]; $sex = $usr['sex']; ?>
        <div class="feed-item col-xxs-12 col-xs-6 col-sm-6 col-md-4 col-lg-3 col-auto-height" data-id="<?php echo ($v['id']); ?>">
          <div class="thumbnail">
<?php if($v['type'] == 3): ?><a><video src="http://feed.chujianapp.com/<?php echo ($v['resource']); ?>" poster="http://feed.chujianapp.com/<?php echo ($v['thumb']); ?>" preload="none" controls></a>
<?php else: ?>
...
<?php if(!$v['uid']): ?><a>游客</a>
<?php else: ?>
                <a href="<?php echo U('user_base/view?uid='.$v['uid']);?>" target="_blank" class="popover-avatar"><?php echo ($v['uid']); ?></a>
...
<?php if($v['audit_aid']): ?><p class="text-nowrap"><b>审 核 人:</b><?php echo ($data['audit_admins'][$v['audit_aid']]['nickname'] ?: $v['audit_aid']); ?></p><?php endif; ?>
              <p class="text-center">
                <a href="http://bs.baidu.com/adtest/27279a15e3556bee5c47d9fd31edb121.swf?flvurl=http://feed.chujianapp.com/<?php echo ($v['resource']); ?>&imgurl=http://feed.chujianapp.com/<?php echo ($v['thumb']); ?>&isplay=false" target="_blank" class="btn btn-sm btn-white hide">全屏</a>
                <a href="http://image.baidu.com/n/pc_search?queryImageUrl=http://feed.chujianapp.com/<?php echo ($v['thumb']); ?>" target="_blank" class="btn btn-sm btn-white hide">查询盗图</a>
<?php if(!$v['audited']): ?><a href="<?php echo U('avatar_audit?id='.$v['id'].'&audited=1');?>" class="btn btn-sm btn-success act-feed-audit">审核</a>
                <!--a href="<?php echo U('avatar_audit?id='.$v['id'].'&audited=2');?>" class="btn btn-sm btn-primary act-feed-audit">推荐</a--><?php endif; ?>
                <a href="<?php echo U('user_base/view?uid='.$v['uid'].'&tab=tab-user-feed');?>" target="_blank" class="btn btn-sm btn-danger">删除</a>
                <a data-href="http://feed.chujianapp.com/<?php echo ($v['resource']); ?>" class="btn btn-sm btn-white auto-qrcode"><i class="fa fa-2x fa-qrcode"></i></a>
              </p>
            </div>
          </div>
