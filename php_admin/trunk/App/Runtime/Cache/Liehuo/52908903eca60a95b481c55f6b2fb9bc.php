    ...
    <div class="row-fluid">
      <div class="clearfix">
        <form action="<?php echo U();?>" method="GET" class="form-inline pull-left" style="margin-bottom:20px;">
          ...
          <input type="hidden" name="show_type" value="<?php echo ($_REQUEST['show_type']); ?>">
        ...
        <div class="pull-right">
          <a href="<?php echo U('?show_type=admin');?>" class="btn btn-primary">按管理员分组</a>
          <span class="btn btn-white">记录数：<?php echo count($data['list']);?></span>
        </div>
      </div>
      <div class="table-responsive">
        <table class="table table-bordered table-striped table-hover text-center">
          <thead>
            <tr>
              <th>日期</th>
<?php if($_REQUEST['show_type'] == 'admin'): ?><th>客服</th><?php endif; ?>
              <th>用户评级</th>
              <th>照片审核</th>
              <th>文字审核</th>
              <th>答疑</th>
          ...
          <tbody>
<?php if(is_array($data['list'])): $i = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; $par = [ 'aid' => $v['aid'], 'stime' => $v['date'], 'etime' => $v['date'], ]; ?>
            <tr>
              <td><?php echo ($v['date']); ?></td>
<?php if($_REQUEST['show_type'] == 'admin'): ?><td><?php echo ($data['admins'][$v['aid']]['nickname'] ?: $v['aid']); ?></td><?php endif; ?>
              <td>
                <a href="<?php echo U('UserBase/oper_logs?type=scoring',$par);?>" target="_blank"><?php echo ($v['cnt_scoring']); ?></a>
              </td>
              <td>
                <a href="<?php echo U('UserBase/oper_logs?type=avatar_audit',$par);?>" target="_blank"><?php echo ($v['cnt_avatar_audit']); ?></a>
              </td>
              <td>
                <a href="<?php echo U('UserBase/oper_logs?type=text_audit',$par);?>" target="_blank"><?php echo ($v['cnt_text_audit']); ?></a>
              </td>
              <td>
                <a href="<?php echo U('UserBase/oper_logs?type=feedback',$par);?>" target="_blank"><?php echo ($v['cnt_feedback']); ?></a>
              </td>
              <td>
                <a href="<?php echo U('UserBase/oper_logs?type=report_handle',$par);?>" target="_blank"><?php echo ($v['cnt_report_handle']); ?></a>
              </td>
