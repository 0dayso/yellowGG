    ...
    <div class="row-fluid">
      <div class="table-responsive">
        <table class="table table-bordered table-striped table-hover text-center">
          ...
          <tbody>
<?php $form = 'form_row_add'; ?>
            <tr class="gradeX">
              <td>添加</td>
              <td>
                <input type="text" name="ch_name" value="" placeholder="必填" form="<?php echo ($form); ?>" class="form-control input-sm">
              </td>
              <td>
                <input type="text" name="ch_serial" value="" placeholder="唯一不可重复" form="<?php echo ($form); ?>" class="form-control input-sm">
              </td>
              <td>
                <input type="text" name="ch_sums" value="" form="<?php echo ($form); ?>" class="form-control input-sm">
              </td>
              <td></td>
              <td>
                <form action="<?php echo U('channel_save');?>" method="POST" id="<?php echo ($form); ?>" class="form-inline">
                  <input type="hidden" name="id" value="">
                  <button type="submit" class="btn btn-primary">添加</button>
                </form>
              </td>
            </tr>
<?php if(is_array($data['list'])): $i = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; $form = 'form_row_'.$v['id']; ?>
            <tr class="gradeX">
              <td><?php echo ($v['id']); ?></td>
              <td>
                <input type="text" name="ch_name" value="<?php echo ($v['ch_name']); ?>" placeholder="必填" form="<?php echo ($form); ?>" class="form-control input-sm">
              </td>
              <td>
                <input type="text" name="ch_serial" value="<?php echo ($v['ch_serial']); ?>" placeholder="必填" form="<?php echo ($form); ?>" class="form-control input-sm">
              </td>
              <td>
                <input type="text" name="ch_sums" value="<?php echo ($v['ch_sums']); ?>" form="<?php echo ($form); ?>" class="form-control input-sm">
              </td>
              <td><?php echo ($v['ch_time'] ? date('Y-m-d H:i:s',$v['ch_time']) : '-'); ?></td>
              <td>
                <form action="<?php echo U('channel_save');?>" method="POST" id="<?php echo ($form); ?>" class="form-inline">
                  <input type="hidden" name="id" value="<?php echo ($v['id']); ?>">
                  <button type="submit" class="btn btn-primary">保存</button>
                </form>
                <a href="<?php echo U('channel_del?id='.$v['id']);?>" class="btn btn-danger" onclick="return confirm('确定这么做？')">删除</a>
              </td>
