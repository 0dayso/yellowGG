...
<?php if(is_array($data['states'])): $i = 0; $__LIST__ = $data['states'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"<?php echo $_REQUEST['state'] == (string)$key ? ' selected' : ''; ?>><?php echo ($v); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
          </div>
          <div class="form-group">
            <input type="text" name="kwd" value="<?php echo ($_REQUEST['kwd']); ?>" placeholder="昵称、用户ID..." class="form-control">
          </div>
      ...
      <div class="table-responsive">
        <table class="table table-bordered table-striped table-hover text-center">
          ...
          <tbody>
<?php if(is_array($data['list'])): $i = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; $log = $data['logs'][$v['uid']] ?: []; ?>
            <tr class="gradeX">
              <td><a href="<?php echo U('UserBase/view?uid='.$v['uid']);?>" target="_blank" class="label label-default"><?php echo ($v['uid']); ?></a></td>
              <td><?php echo ($data['states'][$log['status']]); ?></td>
              <td><?php echo ($data['admins'][$log['aid']]['nickname'] ?: $log['aid']); ?></td>
              <td><?php echo ($log['remark']); ?></td>
              <td>
                <div class="td-content popover-hover"><?php echo ($log['msg']); ?></div>
              </td>
              <td><?php echo ($log['report_id'] ? '举报' : '风控'); ?></td>
              <td><?php echo date('Y-m-d H:i:s',$log['create_time']);?></td>
              <td>
                <a href="<?php echo U('UserBase/view?uid='.$v['uid'].'&tab=tab-user-closure');?>" target="_blank" class="btn btn-primary">解除</a>
              </td>
