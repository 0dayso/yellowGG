    ...
    <div class="row-fluid">
      <div class="clearfix">
        <form action="<?php echo U();?>" method="GET" class="form-inline pull-left">
          ...
          <input type="hidden" name="pub_key" value="<?php echo ($_REQUEST['pub_key']); ?>">
        ...
        <div class="pull-right">
          ...
          <span class="btn btn-white">人数：<?php echo ($data['cnt_user']); ?></span>
      ...
      <div class="table-responsive">
        <table class="table table-bordered table-striped table-hover text-center">
          ...
          <tbody>
<?php if(is_array($data['list'])): $i = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; $cls = ['','success','danger']; $usr = $data['users'][$v['uid']] ?: []; $sex = $usr['sex']; ?>
            <tr class="<?php echo ($cls[$v['state']] ?: 'default'); ?>">
              <td><?php echo ($v['id']); ?></td>
              <td>
                <a href="<?php echo U('UserBase/view?uid='.$v['uid']);?>" target="_blank" class="label label-default"><?php echo ($v['uid'] ?: ''); ?></a>
                <b class="label label-danger"><?php echo boolval($usr['vip_level'] && $usr['vip_valid_end'] >= time()) ? 'v' : ''; echo ($usr['glory_grade'] ?: ''); ?></b>
              </td>
              <td><?php echo (C("USER_SEX_IS.$sex")); ?></td>
              <td><?php echo ($v['pub_key']); ?></td>
              <td class="text-nowrap" style="background-image:linear-gradient(to right,#ded <?php echo ($v['diamond_refund'] / $v['diamond'] * 100); ?>%,rgba(0,0,0,0) 0.01%);"><?php echo ($v['diamond']); echo $v['diamond_refund'] ? ('<br><b class="text-danger">(退还:'.$v['diamond_refund'].')</b>') : ''; ?></td>
              <td><?php echo ($v['glamour']); ?></td>
              <td style="background-image:linear-gradient(to right,#ded <?php echo ($v['cnt_glamour'] / $v['glamour'] * 100); ?>%,rgba(0,0,0,0) 0.01%);"><?php echo ($v['cnt_glamour']); ?></td>
              <td><?php echo ($v['amount']); ?></td>
              <td style="background-image:linear-gradient(to right,#ded <?php echo ($v['cnt_subs'] / $v['amount'] * 100); ?>%,rgba(0,0,0,0) 0.01%);"><?php echo ($v['cnt_subs']); ?></td>
              <td><?php echo ($v['remark']); ?></td>
              <td><?php echo ($v['create_time'] ? date('Y-m-d H:i:s',$v['create_time']) : '-'); ?></td>
              <td><?php echo ($v['finish_time'] ? date('Y-m-d H:i:s',$v['finish_time']) : '-'); ?></td>
              <!--td><?php echo ($v['last_time'] ? date('Y-m-d H:i:s',$v['last_time']) : '-'); ?></td-->
              <td><a href="<?php echo U('lucky_bag_subs?pub_id='.$v['id']);?>" target="_blank" class="btn btn-primary">领取记录</a></td>
