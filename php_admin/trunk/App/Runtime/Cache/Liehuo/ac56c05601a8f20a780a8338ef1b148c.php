...
<div id="content">
  <div class="container-fluid">
    <div class="row-fluid">
      <div class="clearfix">
<?php $dat = $data['item'] ?: []; $adt = $dat['attrs'] ?: []; ?>
        <form action="<?php echo U('task_save');?>" method="POST" class="form-inline">
          <input type="hidden" name="id" value="<?php echo ($dat['id']); ?>">
          <div class="form-group">
            <div class="input-group">
              <span class="input-group-addon">任务名称</span>
              <input type="text" name="title" value="<?php echo ($dat['title']); ?>" placeholder="必填..." required class="form-control">
            </div>
            <div class="input-group">
              <span class="input-group-addon">开始时间</span>
              <input type="text" name="start_time" value="<?php echo ($dat['start_time'] ? date('Y-m-d H:i:s',$dat['start_time']) : ''); ?>" placeholder="必填" class="form-control date-time">
            </div>
            <div class="input-group">
              <span class="input-group-addon">结束时间</span>
              <input type="text" name="end_time" value="<?php echo ($dat['end_time'] ? date('Y-m-d H:i:s',$dat['end_time']) : ''); ?>" placeholder="必填" class="form-control date-time">
            </div>
            <div class="input-group">
              <span class="input-group-addon">优先级</span>
              <input type="text" name="sort" value="<?php echo ($dat['sort']); ?>" placeholder="" class="form-control tip" data-original-title="数值越大优先级越高" style="max-width:4em;">
            </div>
            <div class="form-group">
              <label>
                <input type="checkbox" name="daily" value="1"<?php echo boolval($dat['daily'] == '1') ? ' checked' : '';?>>
                日常任务
              </label>
            </div>
          </div>
...
<?php if(is_array($data['types'])): $i = 0; $__LIST__ = $data['types'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"<?php echo boolval($dat['type'] == (string)$key) ? ' selected' : '';?>><?php echo ($v); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
          </div>
          <div class="form-group" data-filter="<?php echo Liehuo\Model\TaskModel::TYPE_LIVE_WATCH;?> <?php echo Liehuo\Model\TaskModel::TYPE_LIVE_SHOW;?>">
            <div class="input-group">
              <span class="input-group-addon">时长</span>
              <input type="text" name="attrs[duration]" value="<?php echo ($adt['duration']); ?>" placeholder="必填..." required class="form-control" style="max-width:5em;">
              <span class="input-group-addon">分钟</span>
            </div>
          </div>
          <div class="form-group" data-filter="<?php echo Liehuo\Model\TaskModel::TYPE_LIVE_TIMES;?>">
            <div class="input-group">
              <span class="input-group-addon">次数</span>
              <input type="text" name="attrs[times]" value="<?php echo ($adt['times']); ?>" placeholder="必填..." required class="form-control" style="max-width:5em;">
            </div>
          </div>
          <div class="form-group" data-filter="<?php echo Liehuo\Model\TaskModel::TYPE_LIVE_GIFT_RECV;?>">
            <div class="input-group">
              <span class="input-group-addon">收到</span>
              <input type="text" name="attrs[times]" value="<?php echo ($adt['times']); ?>" placeholder="必填..." required class="form-control" style="max-width:5em;">
              <span class="input-group-addon">个</span>
              <select name="attrs[gift_id]" class="form-control">
<?php $arr = [ '3001' => '玫瑰', '3002' => '黄瓜', ]; ?>
<?php if(is_array($arr)): $i = 0; $__LIST__ = $arr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"<?php echo boolval($adt['gift_id'] == (string)$key) ? ' selected' : '';?>><?php echo ($v); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
              </select>
            </div>
          </div>
          <div class="form-group" data-filter="<?php echo Liehuo\Model\TaskModel::TYPE_LIVE_GIFT_SUM;?> <?php echo Liehuo\Model\TaskModel::TYPE_LIVE_GIFT_SEND;?>">
            <div class="input-group">
              <span class="input-group-addon">金额</span>
              <input type="text" name="attrs[amount]" value="<?php echo ($adt['amount']); ?>" placeholder="必填..." required class="form-control" style="max-width:5em;">
              <span class="input-group-addon">个钻石</span>
            </div>
          </div>
          <div class="form-group" data-filter="<?php echo Liehuo\Model\TaskModel::TYPE_LIVE_FOLLOW;?>">
            <div class="input-group">
          ...
          <div class="form-group" data-filter="<?php echo Liehuo\Model\TaskModel::TYPE_LIVE_SHARE;?>">
            <div class="input-group">
          ...
          <div class="form-group" data-filter="<?php echo Liehuo\Model\TaskModel::TYPE_GLORY_GRADE;?>">
            <div class="input-group">
              <span class="input-group-addon">等级达到</span>
              <input type="text" name="attrs[required]" value="<?php echo ($adt['required']); ?>" placeholder="必填..." required class="form-control" style="max-width:5em;">
            </div>
          </div>
          <div class="form-group">
            <div class="input-group">
              <span class="input-group-addon">奖励</span>
              <input type="text" name="attrs[bonus_min]" value="<?php echo ($adt['bonus_min']); ?>" placeholder="最少" class="form-control" style="max-width:4em;">
              <span class="input-group-addon">-</span>
              <input type="text" name="attrs[bonus_max]" value="<?php echo ($adt['bonus_max']); ?>" placeholder="最多" class="form-control" style="max-width:4em;">
              <span class="input-group-addon">个</span>
              <select name="attrs[reward]" class="form-control filter-fields" data-type="filter-reward">
<?php $arr = [ 'diamond' => '钻石', 'glory_pkg' => '荣耀等级礼包', ]; ?>
<?php if(is_array($arr)): $i = 0; $__LIST__ = $arr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"<?php echo boolval($adt['reward'] == (string)$key) ? ' selected' : '';?>><?php echo ($v); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                <option value=""<?php echo boolval($adt['reward'] === '') ? ' selected' : '';?>>其他</option>
              </select>
            </div>
          </div>
...
<?php $arr = [ 6 => '礼包6', 8 => '礼包8', 10 => '礼包10', 11 => '礼包11', 12 => '礼包12', 13 => '礼包13', 15 => '礼包15', 16 => '礼包16', 17 => '礼包17', 18 => '礼包18', 19 => '礼包19', 20 => '礼包20', 21 => '礼包21', 22 => '礼包22', 23 => '礼包23', 24 => '礼包24', 25 => '礼包25', ]; ?>
<?php if(is_array($arr)): $i = 0; $__LIST__ = $arr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"<?php echo boolval($adt['glory_pkg'] == (string)$key) ? ' selected' : '';?>><?php echo ($v); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
              </select>
            </div>
          </div>
          <div class="form-group">
            <div class="input-group">
              ...
              <input type="text" name="attrs[reward_tip]" value="<?php echo ($adt['reward_tip']); ?>" placeholder="选填" class="form-control">
            </div>
          </div>
          <div class="form-group" data-filter="<?php echo Liehuo\Model\TaskModel::TYPE_LIVE_WATCH;?>">
            <label>
              <input type="checkbox" name="attrs[is_vip]" value="1"<?php echo boolval($adt['is_vip'] == '1') ? ' checked' : '';?>>
              会员专属
            </label>
          </div>
          <hr style="margin:5px 0;">
          <div class="form-group">
            <div class="input-group">
              ...
              <textarea name="attrs[detail]" rows="1" placeholder="选填" class="form-control" style="min-width:800px;"><?php echo ($adt['detail']); ?></textarea>
            </div>
          </div>
          <button type="submit" class="btn btn-primary">保存</button>
<?php if($_REQUEST['id']): ?><a href="<?php echo U();?>" class="btn btn-success">新增</a><?php endif; ?>
        </form>
      </div>
      <hr>
...
<?php if(is_array($data['types'])): $i = 0; $__LIST__ = $data['types'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"<?php echo boolval($_REQUEST['type'] == (string)$key) ? ' selected' : '';?>><?php echo ($v); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
          </div>
      ...
      <div class="table-responsive">
        <table class="table table-bordered table-striped table-hover text-center">
          ...
          <tbody>
<?php $ans = [ 'times' => '次数', 'amount' => '数量', 'duration' => '时长', 'required' => '需要达到', 'reward' => '奖励类型', 'bonus_min' => '最少奖励', 'bonus_max' => '最多奖励', 'reward_pkg'=> '奖励礼包', 'reward_tip'=> '奖励提示', 'is_vip' => '会员任务', 'detail' => '任务详情', ]; ?>
<?php if(is_array($data['list'])): $i = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
              ...
              <td>
                <?php echo ($data['types'][$v['type']] ?: $v['type']); ?>
                <b class="label label-success"><?php echo ($v['daily'] ? '日常' : ''); ?></b>
              </td>
              <td>
                <div class="td-content">
<?php echo implode('',array_map(function($k,$v) use($ans) { if(!$v) return ''; return ($ans[$k] ?: $k).':'.$v.'<br>'; },array_keys($v['attrs']),$v['attrs'])); ?>
                </div>
              </td>
              ...
              <td class="text-nowrap">
                <a href="<?php echo U('tasks?id='.$v['id']);?>" class="btn btn-primary">编辑</a>
                <a href="<?php echo U('task_del?id='.$v['id']);?>" class="btn btn-danger" onclick="return confirm('真的要这么做？')">删除</a>
              </td>
    ...
    <div class="pagination alternate">
      <ul>
        <li style="text-align:center;color:#00f;"><?php echo ($pager ? $pager->show() : $page); ?></li>
      </ul>
    </div>
  </div>
</div>
